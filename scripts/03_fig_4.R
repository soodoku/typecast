...
tidy_pcomp <- pcomp %>%
	filter(bayes_cond != "") %>%
	select(bayes_cond, bayes_rep_rich_dv, bayes_rep_evang_dv, bayes_dem_lgb_dv, bayes_dem_union_dv) %>%
...
ggplot(tidy_pcomp, aes(bayes_cond, so_mean)) +
  ...
  scale_y_continuous("Perceived Share", limits = c(0, 80), labels = function(x) paste0(x, "%")) + 
...
ggsave("figs/misinfo_bayes_fig.pdf", height=14, width=14)
...
small_pcomp <- pcomp %>%
	filter(bayes_cond != "") %>%
	select(bayes_cond, bayes_rep_rich_dv, bayes_rep_evang_dv, bayes_dem_lgb_dv, bayes_dem_union_dv, responseid, br_treat, gc_treat, num_index) %>%
	gather(var_name, var_value, bayes_rep_rich_dv:bayes_dem_union_dv, -bayes_cond, -responseid, -br_treat, -gc_treat, -num_index)

with(small_pcomp, lmer(var_value/100 ~ br_treat + gc_treat + (1|responseid) + (1|var_name)))
with(small_pcomp, lmer(var_value/100 ~ br_treat*num_index + gc_treat*num_index + (1|responseid) + (1|var_name)))
